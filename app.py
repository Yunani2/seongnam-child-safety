...
<script>
  ...
  function showToast(msg) {{
    ...
    t._tid = setTimeout(function() {{ t.style.display = 'none'; }}, 3000);
        ...
        # GitHub Pages에 호스팅된 로드뷰 HTML을 iframe으로 임베드
        ROADVIEW_BASE = "https://yunani2.github.io/seongnam-child-safety/roadview.html"
        ...
        if naver_client_id:
            ...
            zone_name = selected_row.get("대상시설명", "") if selected_row is not None else ""
            url = f"{ROADVIEW_BASE}?lat={lat}&lng={lon}&key={naver_client_id}"
            st.components.v1.iframe(url, height=460, scrolling=False)
            if zone_name:
